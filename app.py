...
class DesignInputs:
    ...
    plans_per_nic: int = 1       # 1, 2, or 4; total fabrics = plans_per_nic * nics_per_gpu
...
def design_fabric(inp: DesignInputs) -> DesignResult:
    ...
    # --- Validate speeds --------------------------------------------------
    if inp.leaf_speed < inp.nic_speed or inp.leaf_speed % inp.nic_speed != 0:
        return _infeasible(inp, notes + ["Leaf port speed must be >= NIC speed "
    ...
    # Breakouts
    leaf_breakout = inp.leaf_speed // inp.nic_speed
...
def _single_switch_result(inp: DesignInputs, num_planes: int,
    ...
    if leaf_breakout > 1:
        notes.append(
            ...
            f"{leaf_breakout} x {inp.nic_speed}G NIC links."
        )
    total_nodes = math.ceil(inp.num_gpus / inp.gpus_per_node)
    notes.append(
        f"Nodes: {total_nodes} total (each with {inp.gpus_per_node} GPUs and "
        f"{inp.nics_per_gpu} x {inp.nic_speed}G NIC(s))."
    )
    if num_planes > 1:
        notes.append(
...
def _infeasible(inp: DesignInputs, notes: list[str]) -> DesignResult:
    plane = PlaneDesign(
        nic_speed=inp.nic_speed, leaf_speed=inp.leaf_speed,
    ...
    return DesignResult(
        inputs=inp, num_planes=inp.plans_per_nic * inp.nics_per_gpu, plane=plane,
...
def _add_common_notes(notes: list[str], inp: DesignInputs, leaf_breakout: int,
    ...
    if leaf_breakout > 1:
        notes.append(
            ...
            f"into {leaf_breakout} x {inp.nic_speed}G NIC links."
        )
    ...
    for slot, lx in zip(leaf_slots, leaf_xs):
        ...
        for j in range(nodes_per_leaf_draw):
            ...
            parts.append(
                ...
                f'font-size="10">{result.inputs.nics_per_gpu}x{_fmt_speed(plane.nic_speed)} NIC</text>'
            )
